...
parquet_file_path = Path(__file__).resolve().parent.parent / '__DUCKdb_1' / \
    '_inputs'/'pqt_folder'/'SOLUSDT-aggTrades-2026-07-08.parquet'

#-------------- This ENABLES READING MULTIPLE parquet files -----------------
# wildcard path --  for multiple parquet files
pqt_folder = Path(__file__).resolve().parent.parent / \
    '__DUCKdb_1'/'_inputs'/'pqt_folder'
wildcard_path = pqt_folder/'*.parquet'
#-------------------------------------------------------------------------
...
WITH raw_data AS (
    SELECT 
        ...
        is_buyer_maker
    FROM read_parquet('{wildcard_path.as_posix()}')
),
